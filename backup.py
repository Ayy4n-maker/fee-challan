...
def _build(month=None, year=None):
    ...
    report_payments = [p for p in payments if str(p.get("status", "")).upper() == "PAID"]
    report_expenses = expenses
    report_challans = challans
    if monthly:
        report_payments = [p for p in report_payments if _date_match(p, month, year)]
    ...
    revenue = sum(_money(p, "amount_received", "amount_paid", "total_payable", "total_amount") for p in report_payments)
    late = sum(_money(p, "late_fee", "late_charges") for p in report_payments)
    annual = sum(_money(p, "annual_fee", "annual_charges") for p in report_payments)
    stationery = sum(_money(p, "stationery_fee", "exam_fee", "stationery_exam_fee") for p in report_payments)
    admission = sum(_money(p, "admission_fee", "additional_fee") for p in report_payments)
    other = sum(_money(p, "other_fee", "other_charges") for p in report_payments)
    discount = sum(_money(p, "discount") for p in report_payments)
    ...
    for label, count in [("Students", len(students)), ("Teachers", len(teachers)), ("Paid Payments", len(report_payments)), ("Expenses", len(report_expenses)), ("Challans", len(report_challans))]:
