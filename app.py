...
def class_sort_order(class_name):
    ...
    return class_order.get(class_name, 999)


# ==========================================================
# MULTI-TENANT PORTFOLIO & USER AUTHENTICATION
# ==========================================================
...
def get_user_by_credentials(username, password, role):
    ...
    if (
        role == "admin"
        and uname == ADMIN_USERNAME.strip().lower()
    ...
    if (
        role == "staff"
        and uname == STAFF_USERNAME.strip().lower()
...
def login_required(function):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not (
            ...
            or session.get("staff_logged_in") is True
        ):
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            session["portfolio_id"] = user.get(
                "portfolio_id",
                DEFAULT_PORTFOLIO_ID
            )

            session["school_name"] = user.get(
                "school_name",
                "Irshad Educational Academy"
            )
...
def admin():
    ...
    payments_docs = get_portfolio_docs(
        "fee_payments"
    )
    ...
    expenses_docs = get_portfolio_docs(
        "expenses"
    )

    recorded_expenses = sum(
        float(
            (doc.to_dict() or {}).get("amount") or 0
        )
        for doc in expenses_docs
    )

    teachers_docs = get_portfolio_docs(
        "teachers"
    )

    total_teacher_salaries = sum(
        float(
            (doc.to_dict() or {}).get("salary") or 0
        )
    ...
    try:
        ...
        print(
            f"Error fetching staff user credentials: {e}"
        )
    ...
    return render_template(
        ...
        current_school_name=session.get(
            "school_name",
            "Irshad Educational Academy"
        ),
...
def update_staff_credentials():
    ...
    new_staff_username = request.form.get(
        "staff_username",
        ""
    ).strip()

    new_staff_password = request.form.get(
        "staff_password",
        ""
    ).strip()
    ...
    try:
        ...
        print(
            f"Error updating staff credentials: {e}"
        )
...
def register():
    ...
    if request.method == "POST":
        ...
        school_name = request.form.get(
            "school_name",
            ""
        ).strip() or "Irshad Educational Academy"

        staff_username = request.form.get(
            "staff_username",
            ""
        ).strip()

        staff_password = request.form.get(
            "staff_password",
            ""
        ).strip()
        ...
        try:
            ...
            print(
                f"Error checking usernames: {e}"
            )

        portfolio_id = (
            f"port_{uuid.uuid4().hex[:10]}"
        )
...
def generate_all_challans():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            fee_month = parsed_date.strftime(
                "%B %Y"
            )
...
def add_teacher():
    ...
    if request.method == "POST":
        ...
        allotted_class = request.form.get(
            "allotted_class",
            ""
        )

        allotted_subjects = request.form.get(
            "allotted_subjects",
            ""
        )
